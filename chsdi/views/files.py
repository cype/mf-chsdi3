...
def _update_item(admin_id):
    ...
    try:
        ...
        return False
    try:
        item['timestamp'] = time.strftime('%Y-%m-%d %X', time.localtime())
        item.save()
    except Exception as e:
...
class FileView(object):
    ...
    def _save_to_s3(self, data, mime):
        try:
            k = Key(self.bucket)
            k.key = self.file_id
            k.set_metadata('Content-Type', mime)
            k.set_contents_from_string(data, replace=False)
            _add_item(self.admin_id, self.file_id)

        except:
            raise exc.HTTPInternalServerError('Cannot create file')
